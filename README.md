...
| `--unattended`           | Inject skip/trust/blanket-allow permission defaults so agents do not prompt                        | config file (`off`)    |
...
CLI flags override config file values. `--unattended` enables the permission defaults; if `unattended: true` is set in config, there is no command-line off switch. `--prevent-sleep` accepts `on`/`off` as well as `true`/`false`; the config file always uses a boolean.
`--unattended` is a boolean flag; set `unattended: true` in config to persist the same overnight permission defaults.
...
- By default, `gnhf` does not inject skip/trust/blanket-allow flags. Pass `--unattended` or set `unattended: true` to restore the previous non-interactive defaults (`claude --dangerously-skip-permissions`, `codex --dangerously-bypass-approvals-and-sandbox`, `copilot --allow-all`, `cursor --force --trust --approve-mcps`, OpenCode `permission * allow`, and ACP `approve-all`). If you set a permission or execution-mode flag yourself via `agentArgsOverride`, `gnhf` treats that as user-managed and does not add its `--unattended` default on top.
...
| ------------------ | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude Code        | `--agent claude`                  | Install Anthropic's `claude` CLI and sign in first.                                                                                                                                 | `gnhf` invokes `claude` directly in non-interactive mode. Permission defaults follow the `unattended` setting described above. After Claude emits a successful structured result, `gnhf` treats that result as final and shuts down any lingering Claude process tree after a short grace period.                                                                                 |
| Codex              | `--agent codex`                   | Install OpenAI's `codex` CLI and sign in first.                                                                                                                                     | `gnhf` invokes `codex exec` directly in non-interactive mode. Permission defaults follow the `unattended` setting described above.                                                                                                                                                                                                                       |
| GitHub Copilot CLI | `--agent copilot`                 | Install GitHub Copilot CLI and sign in first.                                                                                                                                       | `gnhf` invokes `copilot` directly in non-interactive JSONL mode. Permission defaults follow the `unattended` setting described above. Copilot currently exposes assistant output tokens, but not full input/cache token totals; see https://github.com/github/copilot-cli/issues/1152.                                                                                                       |
...
| OpenCode           | `--agent opencode`                | Install `opencode` and configure at least one usable model provider first.                                                                                                          | `gnhf` starts a local `opencode serve --hostname 127.0.0.1 --port <port> --print-logs` process automatically and creates a per-run session. Its permission defaults follow the `unattended` setting described above.                                                                                                                                                                                           |
| ACP target         | `--agent acp:<target-or-command>` | Install and authenticate the target supported by the bundled [`acpx`](https://github.com/openclaw/acpx) registry, such as `acp:gemini`, or pass a quoted custom ACP server command. | `gnhf` runs the target through ACP with a persistent per-run session under `.gnhf/runs/<runId>/acp-sessions`; its permission mode follows the `unattended` setting described above. Token usage and `--max-tokens` use ACP `used` deltas when available, with prompt-length plus tool-call estimates as a fallback, and `agentPathOverride` and `agentArgsOverride` do not apply.                           |
